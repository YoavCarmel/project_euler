def ans():
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i]) - streak + 1):
            p = 1
            ...
            if p > max_p:
                max_p = p
    # column
    for i in range(len(grid[0]) - streak + 1):
        for j in range(len(grid)):
            ...
            if p > max_p:
                max_p = p
    # diagonal /
    for i in range(len(grid) - streak + 1):
        for j in range(streak - 1, len(grid[i])):
            p = 1
            for k in range(streak):
                p *= grid[i + k][j - k]
            if p > max_p:
    ...
    # diagonal \
    for i in range(len(grid) - streak + 1):
        for j in range(len(grid) - streak + 1):
            p = 1
            for k in range(streak):
                p *= grid[i + k][j + k]
            if p > max_p:
